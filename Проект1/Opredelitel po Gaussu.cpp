...
// элемент, массив, номер строки поиска, измен€ем значение l(line) и с(column)
...
{	
	...
	for (int i = 2; i > -1; i--)
	{
		...
		{		
			...
			// если элемент матрицы делитс€ без остатка. + ѕропускаем элемент который передали в ф-ю, + пропускаем ненужные стобцы
...
// матрица, какую строку вз€ть, из какой строки вычесть
void vichitaem(float matr[3][3], int line, int whatLine)
{
	float saveValue = 0;
